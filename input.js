...
window.addEventListener('keydown', e => {
  if (e.code === 'KeyE') {
    if (inPlane) {
      // Exit plane — bail out at current position
      inPlane = false; player.visible = true;
      player.position.x = playerPlane.position.x + 3;
      player.position.z = playerPlane.position.z;
      ...
      // Board plane
      const ppdx = player.position.x - playerPlane.position.x;
      const ppdz = player.position.z - playerPlane.position.z;
      if (Math.sqrt(ppdx*ppdx+ppdz*ppdz) < 6 && playerPlane.position.y < 2) {
        inPlane = true; player.visible = false; planeSpeed = 0; planeVelY = 0;
      } else {
